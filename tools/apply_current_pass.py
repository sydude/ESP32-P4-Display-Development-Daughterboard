...
METADATA = {
    "D201": {
        "Datasheet": "https://www.bourns.com/docs/product-datasheets/sm8s-q.pdf",
    },
    "L201": {
...
# Native pcbnew placement coordinates. These are isolated parked-cell coordinates;
# mechanically fixed in-board components are intentionally absent.
PLACEMENT = {
    # Vehicle input / filtering / ideal-diode protection
    "J201": (-82.0, -50.0, 0),
    "F201": (-73.0, -44.85, 180),
    "L201": (-56.0, -44.85, 0),
    "C201": (-49.5, -41.5, 90),
    ...
    # LM5176 power train, controller/driver cell, quiet analog strip
    "C309": (13.0, -48.0, 0),
    "C310": (20.0, -42.0, 90),
    "Q301": (28.0, -40.0, -90),
    "Q302": (28.0, -50.0, -90),
    ...
    # Hold-up, input-loss supervisor, sequencer/rails, reset clamp
    "D601": (101.0, 9.0, 0),
    "R601": (108.0, 9.0, 0),
    "C601": (116.0, 9.0, 0),
    "U601": (104.0, 21.0, 0),
...
def set_prop(component, name: str, value: str):
    try:
        component.set_property(name, value); return
    except Exception: pass
    try:
        component.add_property(name, value); return
    except Exception as exc:
        raise RuntimeError(f"Could not set {component.reference}.{name}: {exc}")
...
def patch_project_symbol_library(j801_lib_id: str):
    ...
    if start < 0:
        raise RuntimeError(f"J801 library symbol {name} not found")
    ...
    values = METADATA["J801"]
    for field in ("Value", "Footprint", "Datasheet", "Description", "Manufacturer", "MPN"):
        value = values[field]
        pattern = re.compile(r'(\(property\s+"' + re.escape(field) + r'"\s+")[^"]*(")')
        if pattern.search(block):
            block = pattern.sub(lambda m: m.group(1) + value + m.group(2), block, count=1)
        else:
            insertion = (
...
def run_schematic():
    ...
    cache.add_library_path(str(SYMLIB))
    symdir = os.environ.get("KICAD_SYMBOL_DIR", "")
    if symdir and Path(symdir).exists():
        try: cache.discover_libraries([symdir])
        except Exception: pass
    schematic = Schematic.load(str(SCH))
    before = {c.reference for c in schematic.components}
    ...
    for ref in TESTPOINTS:
        schematic.components.remove_component(ref)
    ...
    j801 = schematic.components.get("J801")
    patch_project_symbol_library(str(j801.lib_id))
    schematic.save()

    check = Schematic.load(str(SCH))
    after = {c.reference for c in check.components}
    ...
    for ref in MOSFETS:
        ...
        if prop(c, "Footprint") != MOSFET_META["Footprint"]:
            raise RuntimeError(f"{ref} footprint mismatch")
    ...
    for c in check.components:
        ref=str(c.reference)
        footprint=str(c.footprint or prop(c,"Footprint"))
        if not ref or ref.startswith("#") or not footprint or not getattr(c,"on_board",True): continue
        rows.append({
            "Reference":ref, "Value":str(c.value), "Manufacturer":prop(c,"Manufacturer"),
            "MPN":prop(c,"MPN"), "Footprint":footprint,
            "DNP":"DNP" if ref in DNP else "POP", "Sheet":sheet_name(ref),
    ...
    (out/"schematic-apply.json").write_text(json.dumps(report,indent=2,ensure_ascii=False))
...
def run_pcb():
    ...
    before_tracks=track_signature(board); before_edges=edge_signature(board); before_zones=len(list(board.Zones()))
    ...
    moved=set(PLACEMENT)
    fixed={r:position_signature(fp) for r,fp in refs.items() if r not in moved and r not in TESTPOINTS}
    ...
    # Replace only the five task groups; preserve NANO-SVG and any unrelated group.
    existing=[]
    ...
    for g in candidates:
        ...
        for meth in ("GetItems","GetMembers"):
            if hasattr(g,meth):
                try:
                    items=list(getattr(g,meth)()); break
                except Exception: items=[]
        for item in items:
            if isinstance(item,pcbnew.FOOTPRINT):
                members.append(item.GetReference()); box=merge_box(box,courtyard_box(item))
    ...
    out=ROOT/"audit-output"; out.mkdir(exist_ok=True)
    (out/"pcb-apply.json").write_text(json.dumps(report,indent=2))
