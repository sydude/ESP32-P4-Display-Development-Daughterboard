...
1. `01_power.kicad_sch` — nominal-12 V input/protection, LM5176-Q1 buck-boost to `SYS_24V`, LM76003 and TPS25947 Nano feed.
...
- The pre-simplification netlist contained 222 physical references; the current netlist contains 197.
- The 25 removed references belong only to the deleted 24 V bench and dual-source ORing architecture; no reference was added.
- A retained-pin comparison found only the intentional `VEH_24V`/`VIN_PROT_24V`→`SYS_24V` and `VEH_PGOOD`→`SYS24_PGOOD` changes.
...
Five placed flags remain. They are schematic-only (`in_bom no`, `on_board no`), have no footprint and do not enter the physical BOM.
...
| `PWR301` | `SYS_24V` | Marks the four-switch LM5176 output after MOSFETs/inductor; the controller pins do not directly drive this rail in ERC terms. |
...
The regenerated BOM contains 197 physical entries: 193 populated and four DNP.

| Category | Count | Treatment |
|---|---:|---|
| Required for normal operation | 153 | Functional power, sequencing, display, touch, DSI, protection and interconnect parts. |
| Development/test/service | 40 | 32 test points; J501/J901 service links; R910 measurement link; J1003 debug header; D1001/D1002 indicators and R1006/R1007 series resistors. |
| Optional/DNP | 4 | `R505`, `C907`, `R907`, `R1005`. |
| **Total** | **197** | 193 POP + 4 DNP. |

J501, J901 and R910 are categorized as service hardware but are normally fitted and electrically required in the circuit as drawn.
...
- reconsider normally fitted service jumpers after prototype characterization.

## 6. Footprint coverage

All 197 physical BOM entries resolve to a footprint:

- 187 use standard KiCad 10 footprint libraries;
- 10 use the project-local `Phase2` library;
...
6. `Texas_DRR0012E_WSON-12_3x3mm_P0.5mm_EP1.3x2.5mm`
7. `Texas_DYY0014A_TSOT-23-14`
8. `Texas_PWP0028V_TSSOP-28-1EP_4.4x9.7mm_P0.65mm_EP3.4x9.7mm_Mask2.94x5.62mm_ThermalVias`
...
Of the 197 physical entries, 164 mounted component instances resolve to a 3D body. The remaining 33 are 32 bare plated test pads and the J501 solder jumper, for which no mounted body exists or is useful. There are zero broken model paths and no mounted-body component lacking a model.
...
- validate 12 V and USB transitions/reverse current with current-limited sources;
