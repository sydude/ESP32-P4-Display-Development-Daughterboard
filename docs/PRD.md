...
The Phase 2 schematic is populated and the first mechanical/floorplanning placement is complete. The PCB remains unrouted; do not begin trace routing, pours, stackup freeze, differential tuning or thermal-via finalization until that work is accepted separately.
...
| Stacking sockets      | Samtec SSQ-113-01-G-D                             | 2×13, 2.54 mm vertical female socket; bottom-mounted on daughterboard | Selected at J1001/J1002. Nominal facing-PCB separation is 11.01 mm with the Nano headers; USB-A/RJ45 pass through local cutouts. P2 remains electrically unused; standoffs carry structural loads. |
...
| UQ-11 NARROWED / PHYSICAL | Which exact socket height, support and cable geometry complete the stack? | `SSQ-113-01-G-D` is selected and placed. It gives 11.01 mm nominal facing-PCB separation; local USB-A/RJ45 cutouts clear the taller parts. Real placement establishes a 130 × 100 mm nominal REV1 envelope, four Nano-aligned M2.5-class supports and two left-wing M3 chassis supports. | Mate the exact sockets, verify measured separation/simultaneous insertion and standoffs, mock the DSI pass-through, and check the tight 0.20 mm nominal RJ45-side margin before mechanical freeze. |
